...
20. ### چطوری از ref استفاده کنیم؟
    ...
    2. اگه از نسخه ۱۶.۸ به بالاتر هم استفاده می‌کنید که یه هوک به اسم useRef هست که . For example, the search bar component's input element accessed as follows,
...
176. ### مدل ذهنی redux-saga چطوریه؟
     ...
     _Saga_ مثل یه thread جداگانه برای برنامه عمل می‌کنه, that's solely responsible for side effects. `redux-saga` is a redux _middleware_, which means this thread can be started, paused and cancelled from the main application with normal Redux actions, it has access to the full Redux application state and it can dispatch Redux actions as well.
